...
export default function BasicAccordion({
  ...
  return (
    <div
      className={`flex w-full flex-col divide-y divide-border overflow-hidden rounded-xl border p-6 bg-card ${className}`}
    >
      {items.map((item) => {
        ...
        return (
          <div className="overflow-hidden" key={item.id}>
            <button
              ...
              className="flex min-h-[44px] w-full items-center justify-between gap-2 px-4 py-4 text-left rounded-xl transition-colors hover:bg-card focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
            ...
            <AnimatePresence initial={false}>
              {isExpanded && (
                <motion.div
                  ...
                  role="region"
                >
                  <div className="border-t font-medium text-foreground rounded-xl px-4 py-6">
                    {item.content}
                  </div>
                </motion.div>
              )}
